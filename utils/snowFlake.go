...
const (
	numberBits uint8 = 12 // 序列号 表示每个集群下的每个节点，1毫秒内可生成的id序号的二进制位
	...
	timeShift uint8 = workerBits + numberBits // 时间戳向左的偏移量（从右向左）
	workerShift uint8 = numberBits // 节点ID向左的偏移量（从右向左）
	// 41位字节作为时间戳数值的话，大约68年就会用完
	// 假如你2010年1月1日开始开发系统 如果不减去2010年1月1日的时间戳 那么白白浪费40年的时间戳啊！
	// 这个一旦定义且开始生成ID后千万不要改了 不然可能会生成相同的ID
	epoch int64 = 1616049117000
)
